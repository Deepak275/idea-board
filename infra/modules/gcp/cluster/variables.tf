# infra/modules/gcp/cluster/variables.tf
#
...
#
# node_size is the shared t-shirt vocabulary; each cloud maps it to a real
# machine type internally (see main.tf).

variable "name" {
  description = "Name prefix / GKE cluster name (e.g. \"idea-board\")."
  type        = string
}
...
variable "k8s_version" {
  description = "Kubernetes control-plane version / min_master_version (e.g. \"1.29\")."
  type        = string
}
...
variable "node_count" {
  description = "Desired number of worker nodes in the node pool (per zone for regional clusters)."
  type        = number
